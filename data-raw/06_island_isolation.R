...
lapply(isl_miss, function(i){
  ...
  entity.pnt.buff.proj <- st_buffer(entity.pnt.proj, dist = (dist_centroid*1000)+5000)
  ...
  #mainland_sf <- st_transform(mainland_sf, st_crs(paste("+proj=aeqd +lat_0=", cen[2]," +lon_0=", cen[1],sep="")))
  #plot(mainland_sf, col="red", add=T)
  ...
  dist_mainland <- tryCatch({
    mainland.clipped1 <- st_crop(mainland_sf, entity.pnt.buff)
  ...
  df <- data.frame(ulm_ID=islands$ulm_ID[i], d_orig=dist_orig, 
                   ...
                   d_bigger_rough=dist_big_rough, d_bigger=dist_big); gc()
...
files <- list.files("island_dist", full.names=T)
